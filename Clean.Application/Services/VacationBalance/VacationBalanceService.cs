...
public class VacationBalanceService : IVacationBalanceService
{
    ...
    public async Task AutoUpdateVacationBalancesAsync()
    {
        ...
        {
            _logger.LogError(ex, "Error occurred while auto-updating vacation balances.");
        }
    }
    ...
    public async Task<Response<GetVacationBalanceDto>> AddVacationBalanceAsync(AddVacationBalanceDto dto)
    {
        try
        {
            ...
            await _cacheService.RemoveAsync($"vacation_balance_by_employee_{vBalance.EmployeeId}");
        ...
        {
            _logger.LogError(ex, "Error while adding a new vacation balance.");
            return new Response<GetVacationBalanceDto>(HttpStatusCode.InternalServerError, "An error occurred while adding vacation balance.");
        }
    }
    ...
    public async Task<Response<List<GetVacationBalanceDto>>> GetAllVacationBalancesAsync(VacationBalanceFilter filter)
    {
        ...
        try
        {
            var cacheKey = GetVacationBalancesCacheKey(filter, "all_vacation_balances");
            var cached = await _cacheService.GetAsync<List<GetVacationBalanceDto>>(cacheKey);
            if (cached != null)
            {
                return new Response<List<GetVacationBalanceDto>>(HttpStatusCode.OK, "Vacation balances retrieved successfully!", cached);
            }
            ...
            var allBalances = vacationTables.Select(vb => new GetVacationBalanceDto
            {
                ...
                Employee = new GetEmployeeDto
                {
                    ...
                    BaseSalary = vb.Employee.SalaryHistories.OrderByDescending(sh => sh.Month).Select(sh => sh.BaseAmount).FirstOrDefault(),
                    IsActive = vb.Employee.IsActive,
                    DepartmentName = vb.Employee.Department.Name
                },
            }).ToList();

            await _cacheService.SetAsync(cacheKey, allBalances, TimeSpan.FromHours(12));

            return new Response<List<GetVacationBalanceDto>>(HttpStatusCode.OK, allBalances);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error while retrieving all vacation balances.");
            return new Response<List<GetVacationBalanceDto>>(HttpStatusCode.InternalServerError, "An error occurred while retrieving vacation balances.");
        }
    }
    ...
    public async Task<Response<List<GetVacationBalanceDto>>> GetLatestVacationBalancesAsync(VacationBalanceFilter filter)
    {
        try
        {
            var cacheKey = GetVacationBalancesCacheKey(filter, "latest_vacation_balances");
            var cached = await _cacheService.GetAsync<List<GetVacationBalanceDto>>(cacheKey);
            if (cached != null)
            {
                return new Response<List<GetVacationBalanceDto>>(HttpStatusCode.OK, "Latest vacation balances retrieved successfully!", cached);
            }
            ...
            var latestBalances = activeEmployees
                ...
                .Select(e =>
                {
                    var latest = e.VacationBalances.OrderByDescending(vb => vb.Year).First();
            ...
            await _cacheService.SetAsync(cacheKey, latestBalances, TimeSpan.FromHours(12));

            return new Response<List<GetVacationBalanceDto>>(HttpStatusCode.OK, latestBalances);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error while retrieving latest vacation balances.");
            return new Response<List<GetVacationBalanceDto>>(HttpStatusCode.InternalServerError, "An error occurred while retrieving latest vacation balances.");
        }
    }
    ...
    public async Task<Response<GetVacationBalanceDto>> GetVacationBalanceByEmployeeIdAsync(int employeeId)
    {
        try
        {
            var cacheKey = $"vacation_balance_by_employee_{employeeId}";
            var cached = await _cacheService.GetAsync<GetVacationBalanceDto>(cacheKey);
            if (cached != null)
            {
                return new Response<GetVacationBalanceDto>(HttpStatusCode.OK, "Employee's vacation balance retrieved successfully!", cached);
            }
            ...
            await _cacheService.SetAsync(cacheKey, eBalance, TimeSpan.FromHours(12));

            return new Response<GetVacationBalanceDto>(HttpStatusCode.OK, eBalance);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error while retrieving vacation balance by employee ID.");
            return new Response<GetVacationBalanceDto>(HttpStatusCode.InternalServerError, "An error occurred while retrieving vacation balance by employee ID.");
        }
    }
    ...
    public async Task<Response<GetVacationBalanceDto>> GetVacationBalanceByIdAsync(int vacationBalanceId)
    {
        try
        {
            var cacheKey = $"vacation_balance_{vacationBalanceId}";
            var cached = await _cacheService.GetAsync<GetVacationBalanceDto>(cacheKey);
            if (cached != null)
            {
                return new Response<GetVacationBalanceDto>(HttpStatusCode.OK, "Vacation balance retrieved successfully!", cached);
            }

            var vBalance = await _vacationBalanceRepository.GetVacationBalanceByIdAsync(vacationBalanceId);
            if (vBalance is null)
            {
                return new Response<GetVacationBalanceDto>(HttpStatusCode.BadRequest, "No such vacation balance in the system");
            }
            ...
            await _cacheService.SetAsync(cacheKey, vacationBalance, TimeSpan.FromHours(12));

            return new Response<GetVacationBalanceDto>(HttpStatusCode.OK, vacationBalance);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error while retrieving vacation balance by ID.");
            return new Response<GetVacationBalanceDto>(HttpStatusCode.InternalServerError, "An error occurred while retrieving vacation balance by ID.");
        }
    }
    ...
    public async Task<Response<GetVacationBalanceDto>> UpdateVacationBalanceAsync(UpdateVacationBalanceDto dto)
    {
        try
        {
            ...
            {
                return new Response<GetVacationBalanceDto>(HttpStatusCode.BadRequest, "No such vacation balance to update in the system");
            }
            ...
            await _cacheService.RemoveAsync($"vacation_balance_{updatedBalance.Id}");
            await _cacheService.RemoveAsync($"vacation_balance_by_employee_{updatedBalance.EmployeeId}");
            await _cacheService.RemoveByPatternAsync("all_vacation_balances_*");
            await _cacheService.RemoveByPatternAsync("latest_vacation_balances_*");
        ...
        {
            _logger.LogError(ex, "Error while updating vacation balance.");
            return new Response<GetVacationBalanceDto>(HttpStatusCode.InternalServerError, "An error occurred while updating vacation balance.");
        }
    }
    
    private static string GetVacationBalancesCacheKey(VacationBalanceFilter filter, string prefix)
    {
        // Concatenate all filter values into a single string
        return $"{prefix}_" + // <--- Use the unique prefix here
               $"employeeId:{filter.EmployeeId?.ToString() ?? "null"}_" +
               $"year:{filter.Year?.ToString() ?? "null"}_" +
